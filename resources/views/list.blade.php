...
<div class="index1">
    ...
    <div>
        ...
        <p></p>
        <h5>検索条件</h5>
        <p>プレイする時間帯：{{$playtime}}</p>
        <p>プレイするゲーム：{{$playgame}}</p>
    </div>
</div>
...
<section class="indexsec">
    ...
    @foreach($users as $user)
        @if (empty($playtime))
            <a href="{{route('user',$user->user->id)}}"         class="indexa">
            @if ($user->user->image === 'img/default.png')
                <img class="rounded-circle index2img" src=" {{      asset('img/default.png') }}" >
            @else
                <img src="{{ Storage::url   ($user->user->image) }} "   class="index2img">
            @endif
            {{$user->user->name}}
            </a>
        @else
            @if ($user->user->playtime == $playtime)
                <a href="{{route('user',$user->user->id)}}"         class="indexa">
                    @if ($user->user->image === 'img/default.png')
                        <img class="rounded-circle index2img" src=" {{      asset('img/default.png') }}" >
                    @else
    ...
    {{-- <div class="mb-4">
        {{ $users->appends(Request::only('keyword'))->links('pagination::bootstrap-5') }}
    </div> --}}
